...
This includes a burn-in phase to reduce autocorrelation in the produced spins.
...
Evaluation: Ising lattice is sampled using the transformer, and the corresponding energy, magnetization and autocorrelations are computed. These are then compared to the test set produced by the MCMC sampler. Autocorelations are measured by approximating E(sigma_i sigma_{i+r}) for varying r, and at critical temperature it is known that long-range correlations occur. Energy and Magnetization are compared to the MCMC sampler.